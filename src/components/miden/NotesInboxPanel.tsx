...
import { useAccounts, useNotes } from '@miden-sdk/react';
...
/**
 * Committed notes + consumable summary for a selected wallet, backed by `useNotes` from `@miden-sdk/react`.
 */
export function NotesInboxPanel() {
  ...
  const { notes, consumableNotes, noteSummaries, refetch, isLoading, error } = useNotes(
    accountId ? { status: 'committed', accountId } : undefined,
  );

  if (accountsLoading) {
    return (
      <div className="ui-card text-sm text-neutral-600">
        Loading accounts…
      </div>
    );
  }
  ...
  if (accountsError) {
    return (
      <div className="ui-card text-sm text-red-800">
    ...
    <div className="ui-card space-y-4">
      <div>
        ...
        <p className="mt-2 max-w-xl text-sm leading-relaxed text-neutral-600">
          ...
          from <code className="rounded bg-neutral-100 px-1">@miden-sdk/react</code>.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button type="button" variant="secondary" size="sm" onClick={() => void refetch()} disabled={isLoading || !accountId}>
      ...
      <div>
        ...
        {noteSummaries.length === 0 ? (
          ...
          <ul className="space-y-2 text-sm">
            {noteSummaries.map((s) => (
              <li key={s.id} className="rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 font-mono text-xs text-neutral-800">
      ...
      <div>
        <h3 className="text-sm font-medium text-neutral-800">Consumable ({consumableNotes.length})</h3>
        <p className="mt-1 text-xs text-neutral-500">
          Full consumable records are available via the SDK hook; this panel only shows the count here.
        </p>
      </div>
    </div>
  );

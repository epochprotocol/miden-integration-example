...
export function NotesInboxPanel() {
  ...
  const { notes, consumableNotes, noteSummaries, refetch, isLoading, error } =
  ...
  if (accountsLoading) {
    return (
      <div className="ui-card text-sm text-neutral-600">Loading accounts…</div>
    );
  }
    ...
    <div className="ui-card space-y-4">
      <div>
        ...
        <p className="mt-2 max-w-xl text-sm leading-relaxed text-neutral-600">
          Committed notes for the selected wallet. Uses{" "}
          <code className="rounded bg-neutral-100 px-1">useNotes</code> and{" "}
          <code className="rounded bg-neutral-100 px-1">useConsume</code> from{" "}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          {isConsuming
            ? "Consuming…"
            : `Consume all (${consumableNotes.length})`}
        </Button>
        {error && <span className="text-sm text-red-700">{error.message}</span>}
        {consumeError && (
          <span className="text-sm text-red-700">{consumeError.message}</span>
        )}
      </div>
      ...
      <div>
        ...
        {noteSummaries.length === 0 ? (
          <p className="text-sm text-neutral-600">
            No note summaries for this filter.
          </p>
        ) : (
            ...
            {noteSummaries.map((s) => (
              ...
              >
                ...
                <div>
                  {s.assets
                    .map(
                      (a) => `${a.amount.toString()} ${a.symbol ?? a.assetId}`,
                    )
                    .join(", ")}
                </div>
              </li>
